...
namespace SimpleEngine {
	...
	constexpr GLenum usage_to_GLenum(const VertexBuffer::EUsage usage) noexcept {
		switch(usage) {
	...
	size_t BufferLayout::get_stride() const noexcept {
		return m_stride;
	}

	BufferElement::BufferElement(const ShaderDataType _type) noexcept :
		type(_type),
	...
	VertexBuffer::VertexBuffer(const void *data, const size_t size, BufferLayout buffer_layout, const EUsage usage) noexcept :
		m_id(0), m_buffer_layout(std::move(buffer_layout)) {
		glGenBuffers(1, &m_id);
